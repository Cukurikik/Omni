...
export class OmniBrowserMCP {
  ...
  private _registerTools() {
      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description:
              "Mengambil Vercel Agent Browser Bounding Boxes untuk VLM.",
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      if (request.params.name === "navigate_url") {
        console.log(
          `🚀 [MCP-ACTION] Model meminta akses ke URL: ${request.params.arguments?.url}`,
        );
        return {
          content: [
            { type: "text", text: "Navigasi Berhasil. DOM Terdiliver." },
          ],
        };
      }
  ...
  public async connect() {
    ...
    console.log(
      "✅ [MCP-CONNECTED] Server MCP Omni siap melayani Cloud LLM API!",
    );
  }
}
