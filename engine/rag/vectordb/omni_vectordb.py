...
class OmniVectorDB:
    ...
    def __init__(self, db_path: str = ":memory:", collection: str = "default"):
        ...
        self.vectors = []       # List of {id, vector, text, metadata}
        self.partitions = defaultdict(list)  # Milvus IVF partitions
        self.doc_freq = defaultdict(int)     # BM25 document frequency
        self._init_sql(db_path)
        print(f"🗄️ [OMNI-VECTORDB] Collection '{collection}' diinisiasi.")
        print(f"   Mode: {'In-Memory (Chroma-style)' if db_path == ':memory:' else f'Persistent SQLite (pgvector-style): {db_path}'}")
