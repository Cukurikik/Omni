...
function simulateRequest(toolId, endpoint) {
  return new Promise((resolve) => {
    const boundary =
      "----OMNITESTBOUNDARY" + Math.random().toString(36).substring(2);
    const options = {
      hostname: "localhost",
      port: 3000,
      ...
      res.on("end", () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          try {
            ...
            console.log(
              `✅ Success for ${toolId} (${endpoint}): ${data.substring(0, 150)}...`,
            );
            resolve(json.data ? json.data.job_id || json.data.base_id : "OK");
          } catch (e) {
            console.log(
              `✅ Success (but not JSON) for ${toolId}: ${data.substring(0, 50)}`,
            );
            resolve("OK");
          }
        } else {
          console.error(
            `❌ Request failed for ${toolId} (${endpoint}): ${res.statusCode} ${data}`,
          );
          resolve(null);
        }
      });
    ...
    const body = Buffer.concat([
      ...
      Buffer.from(
        `Content-Disposition: form-data; name="omni_file"; filename="test.mp4"\r\n`,
      ),
