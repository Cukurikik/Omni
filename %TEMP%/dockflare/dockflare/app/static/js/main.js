...
function initializeAllTomSelects() {
  ...
  if (manualTunnelSelect) {
    manualTunnelTomSelect = new TomSelect(
      manualTunnelSelect,
      singleSelectOptions,
    );
  }
  ...
  return {
    initialize: function () {
      ...
      if (themeMenuScoped && themeSelectorBtn) {
        ...
        console.error(
          "DockFlare Theme Error: UI elements for theme selector not found.",
        );
      }
    },
  };
    ...
    button.addEventListener("click", async function () {
      try {
        ...
        if (parts.length > 2 && !hostname.startsWith("*.")) {
          modal.querySelector("#edit_manual_subdomain").value = parts
            .slice(0, -2)
            .join(".");
          modal.querySelector("#edit_manual_domain_name").value = parts
            .slice(-2)
            .join(".");
        } else {
        ...
        modal.querySelector("#edit_manual_path_display").value =
          path.startsWith("/") ? path.substring(1) : path;
        modal.querySelector("#edit_manual_path").value = path;
        const service = details.service || "";
        const serviceParts = service.split("://");
        if (serviceParts.length === 2) {
          modal.querySelector("#edit_manual_service_type").value =
            serviceParts[0];
          modal.querySelector("#edit_manual_service_address").value =
            serviceParts[1];
        ...
        const accessGroupSelect = modal.querySelector(
          "#edit_manual_access_group",
        );
        const manualPolicySelect = modal.querySelector(
          "#edit_manual_access_policy_type",
        );
        ...
        if (accessGroupSelect) {
          if (accessGroupSelect.tomselect) {
            ...
            accessGroupSelect.value = selectedGroups.length
              ? selectedGroups[0]
              : "";
          }
        }
        ...
        const zoneOverrideField = modal.querySelector(
          "#edit_manual_zone_name_override",
        );
        if (zoneOverrideField) zoneOverrideField.value = "";

        const noTlsVerifyField = modal.querySelector(
          "#edit_manual_no_tls_verify",
        );
        if (noTlsVerifyField)
          noTlsVerifyField.checked = details.no_tls_verify || false;

        const originServerNameField = modal.querySelector(
          "#edit_manual_origin_server_name",
        );
        if (originServerNameField)
          originServerNameField.value = details.origin_server_name || "";

        const httpHostHeaderField = modal.querySelector(
          "#edit_manual_http_host_header",
        );
        if (httpHostHeaderField)
          httpHostHeaderField.value = details.http_host_header || "";

        const http2OriginField = modal.querySelector("#edit_http2_origin");
        if (http2OriginField)
          http2OriginField.checked = details.http2_origin || false;

        const disableChunkedEncodingField = modal.querySelector(
          "#edit_disable_chunked_encoding",
        );
        if (disableChunkedEncodingField)
          disableChunkedEncodingField.checked =
            details.disable_chunked_encoding || false;

        const matchSniToHostField = modal.querySelector(
          "#edit_match_sni_to_host",
        );
        if (matchSniToHostField)
          matchSniToHostField.checked = details.match_sni_to_host || false;
        ...
        if (tunnelDisplay) {
          ...
          const tunnelName =
            details.tunnel_name || (tunnelId ? "Tunnel" : "N/A");
          if (tunnelId) {
            const shortId =
              tunnelId.length > 12 ? `${tunnelId.slice(0, 12)}…` : tunnelId;
        ...
        await dfAlert(
          t("js.alert.edit_dialog_error"),
          t("js.alert.error_title"),
        );
      }
    });
  });
...
function updateRowFromService(row, service) {
  ...
  const statusBadge = statusCell
    ? statusCell.querySelector(".status-badge")
    : null;
  if (statusBadge) {
...
function connectStateUpdateSource() {
  ...
  eventSource.onerror = function (err) {
    console.error(
      "State update stream connection error. It will be retried automatically by the browser.",
      err,
    );
    eventSource.close();
...
function addLogLine(message, type = "log") {
  ...
  if (
    !initialConnectMessageCleared &&
    logOutput.textContent.includes(t("js.text.connecting_logs"))
  ) {
  ...
  if (type === "status")
    newLogLine.classList.add("text-neutral-content", "opacity-70", "italic");
  else if (type === "error")
    newLogLine.classList.add("text-red-400", "font-semibold");
  else if (type === "connected") newLogLine.classList.add("text-green-400");

  const isScrolledToBottom =
    logOutput.scrollHeight - logOutput.clientHeight <= logOutput.scrollTop + 10;
...
function setupLogControls() {
  ...
  clearBtn.addEventListener("click", () => {
    if (logOutput) {
      logOutput.textContent = activeLogSource
        ? t("js.text.log_cleared") + "\n"
...
function connectEventSource() {
  ...
  try {
    ...
    activeLogSource.onerror = function (err) {
      ...
      if (
        activeLogSource &&
        activeLogSource.readyState !== EventSource.CLOSED
      ) {
      ...
      if (logsEnabled) {
        ...
        const delay = Math.min(
          5000 * Math.pow(1.5, Math.min(retryAttempt - 1, 5)),
          30000,
        );
    ...
    addLogLine(
      t("js.text.log_connection_failed", { error: e.message }),
      "error",
    );
    if (logsEnabled) {
  ...
  eventSourceHealthCheck = setInterval(() => {
    if (
      logsEnabled &&
      (!activeLogSource || activeLogSource.readyState === EventSource.CLOSED)
    ) {
  ...
  document.querySelectorAll("div[data-delete-at]").forEach((div) => {
    ...
    try {
      ...
      if (diffMs < 0) {
        ...
        absoluteTimeSpan.className =
          "absolute-time-display text-error font-bold";
      } else if (diffSeconds < 3600) {
        const minutes = Math.floor(diffSeconds / 60);
        const seconds = diffSeconds % 60;
        const timeStr = `${minutes}:${seconds.toString().padStart(2, "0")}`;

        absoluteTimeSpan.textContent = t("js.text.countdown_expires_in", {
          time: timeStr,
        });
        countdownSpan.textContent = "";
        ...
        if (diffSeconds <= 10) {
          absoluteTimeSpan.className =
            "absolute-time-display text-error font-bold animate-pulse";
        } else if (diffSeconds <= 30) {
          absoluteTimeSpan.className =
            "absolute-time-display text-error font-semibold";
        } else if (diffSeconds <= 120) {
          absoluteTimeSpan.className =
            "absolute-time-display text-warning font-semibold";
        } else {
        ...
        countdownSpan.textContent = "";
        absoluteTimeSpan.className =
          "absolute-time-display text-base-content opacity-70";
      }
  ...
  pingInterval = setInterval(() => {
    fetch(`${document.baseURI}ping?t=${Date.now()}`)
      .then((response) =>
        response.ok
          ? response.json()
          : Promise.reject(`Ping failed: ${response.status}`),
      )
      .then((data) => {})
  ...
  fetch(`${document.baseURI}reconciliation-status?t=${Date.now()}`)
    ...
    .then((data) => {
      ...
      if (data.in_progress) {
        ...
        if (
          statusElement.innerHTML.includes("Reconciliation:") ||
          statusElement.innerHTML.includes(
            t("js.text.reconciliation_progress", { progress: "" }),
          )
        ) {
          ...
          setTimeout(() => {
            if (
              statusElement.innerHTML.includes(
                t("js.text.reconciliation_complete"),
              )
            ) {
    ...
    .catch((err) =>
      console.warn("Failed to fetch reconciliation status:", err),
    );
}
...
function detectZoneForHostname(hostname) {
  ...
  cachedZones.forEach((zone) => {
    ...
    if (!zoneName) return;
    if (
      normalizedHost === zoneName ||
      normalizedHost.endsWith(`.${zoneName}`)
    ) {
      matches.push(zone);
    }
  });
...
function populateZoneSelector(
  ...
  (zones || []).forEach((zone) => {
    ...
    option.textContent = zone.name
      ? `${zone.name} (${zone.id.slice(0, 8)}${zone.id.length > 8 ? "…" : ""})`
      : zone.id;
...
function updateManualZoneUI(state, elements) {
  if (!elements) return;
  const { zoneIdInput, selectorWrapper, selectorEl, messageEl, badgeEl } =
    elements;
  ...
  switch (state.status) {
    ...
    case "override":
      ...
      if (state.zoneName) {
        ...
        messageEl.textContent = t("js.text.zone_override", {
    ...
    case "ok":
      ...
      messageEl.textContent =
        state.zone && state.zone.name
          ? t("js.text.zone_detected", { zoneName: state.zone.name })
          : t("js.text.zone_detected", { zoneName: "" });
      break;
    case "ambiguous":
      zoneIdInput.value = "";
    ...
    case "selected":
      ...
      messageEl.textContent =
        state.zone && state.zone.name
          ? t("js.text.zone_selected", { zoneName: state.zone.name })
          : t("js.text.zone_selected", { zoneName: "" });
      break;
    default:
      break;
  }
  ...
  if (Array.isArray(tunnels) && tunnels.length > 0) {
    tunnels.forEach((tunnel) => {
      ...
      const label = tunnel.name
        ? `${tunnel.name} (${tunnel.id.slice(0, 8)}${tunnel.id.length > 8 ? "…" : ""})`
        : tunnel.id;
...
async function initializeManualRuleForm() {
  ...
  const zoneSelectorWrapper = document.getElementById(
    "manual_zone_selector_wrapper",
  );
  const zoneSelectorEl = document.getElementById("manual_zone_selector");
  const zoneOverrideInput = document.getElementById(
    "manual_zone_name_override",
  );
  ...
  const triggerDetection = async () => {
    const overrideValue = zoneOverrideInput
      ? zoneOverrideInput.value.trim()
      : "";
    if (overrideValue) {
      updateManualZoneUI(
        { status: "override", zoneName: overrideValue },
        elements,
      );
      return;
    }
    const hostname = buildManualHostname(
  ...
  if (subdomainInput)
    subdomainInput.addEventListener("input", scheduleDetection);
  ...
  if (zoneOverrideInput) {
    zoneOverrideInput.addEventListener("input", () => {
      if (zoneOverrideInput.value.trim()) {
        updateManualZoneUI(
          { status: "override", zoneName: zoneOverrideInput.value.trim() },
          elements,
        );
      } else {
        scheduleDetection();
...
async function openEditAccessGroupModal(groupId, details) {
  ...
  title.textContent = t("js.text.edit_access_group_title", {
  ...
  document.getElementById("group_display_name").value =
    details.display_name || "";
  document.getElementById("group_session_duration").value =
    details.session_duration || "24h";
  document.getElementById("group_app_launcher_visible").checked =
    details.app_launcher_visible || false;
  document.getElementById("group_auto_redirect").checked =
    details.auto_redirect_to_identity || false;

  const isPublicMode = details.public_mode === true;
  document.getElementById("public_mode").value = isPublicMode
    ? "true"
    : "false";
    ...
    details.policies.forEach((policy) => {
      if (policy.include) {
        policy.include.forEach((rule) => {
          ...
          else if (rule["login_method"] && rule["login_method"].id)
            selectedIdpIds.push(rule["login_method"].id);
        });
      }
  ...
  if (window.idpTomSelect && selectedIdpIds.length > 0) {
    try {
      ...
      if (data.success && data.identity_providers) {
        ...
        for (const [friendlyName, idpData] of Object.entries(
...
function updateManualRuleServiceFields() {
  const manualServiceTypeSelect = document.getElementById(
    "manual_service_type",
  );
  ...
  const originServerNameDiv = document.getElementById(
    "manual_origin_server_name_div",
  );
  const manualServiceAddressInput = document.getElementById(
    "manual_service_address",
  );
  const manualServiceAddressLabel = document.getElementById(
    "manual_service_address_label",
  );
  const manualServiceHelpText = document.getElementById("manual_service_help");
  const manualServicePrefixSpan = document.getElementById(
    "manual_service_prefix_span",
  );
  const matchSniToHostDiv = document.getElementById(
    "manual_match_sni_to_host_div",
  );
  ...
  if (manualServiceAddressInput)
    manualServiceAddressInput.placeholder = "host:port or status code";
  if (manualServiceAddressLabel)
    manualServiceAddressLabel.textContent = "URL (Required for most types)";
  if (manualServiceHelpText)
    manualServiceHelpText.textContent =
      "e.g., 192.168.1.10:8000 or my-service.local:3000 for HTTP/S/TCP etc.";
  ...
  if (selectedType === "http" || selectedType === "https") {
    ...
    if (manualServiceAddressInput)
      manualServiceAddressInput.placeholder =
        "host:port or resolvable hostname";
    if (manualServiceAddressLabel)
      manualServiceAddressLabel.textContent = "Origin URL (Required)";
    if (manualServiceHelpText)
      manualServiceHelpText.textContent =
        "e.g., 192.168.1.10:8000 or my-service.local:3000";
    ...
    if (manualServiceAddressInput)
      manualServiceAddressInput.placeholder = "host:port";
    if (manualServiceAddressLabel)
      manualServiceAddressLabel.textContent = `Origin Address for ${selectedType.toUpperCase()} (host:port)`;
    if (manualServiceHelpText)
      manualServiceHelpText.textContent = `e.g., my-internal-server:22`;
  } else if (selectedType === "http_status") {
    if (manualServiceAddressInput)
      manualServiceAddressInput.placeholder = "e.g., 404";
    if (manualServiceAddressLabel)
      manualServiceAddressLabel.textContent =
    ...
    if (manualServiceAddressInput)
      manualServiceAddressInput.style.display = "none";
    if (manualServicePrefixSpan)
      manualServicePrefixSpan.classList.add("hidden");
    if (manualServiceAddressLabel)
      manualServiceAddressLabel.textContent = "Origin URL (Not Required)";
    if (manualServiceHelpText)
      manualServiceHelpText.textContent =
        "Bastion mode routes based on the public hostname directly.";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (manualServiceTypeSelect) {
    manualServiceTypeSelect.addEventListener(
      "change",
      updateManualRuleServiceFields,
    );
    updateManualRuleServiceFields(); // Run once on load
    setupPathInput(
      document.getElementById("manual_path_display"),
      document.getElementById("manual_path"),
    );
    setupPathInput(
      document.getElementById("edit_manual_path_display"),
      document.getElementById("edit_manual_path"),
    );
  ...
  if (deleteTunnelModal) {
    ...
    const updateConfirmState = () => {
      ...
      confirmButton.disabled =
        confirmInput.value.trim().toLowerCase() !== "delete";
    };
  ...
  const manualAccessGroupSelect = document.getElementById(
    "manual_access_group",
  );
  const manualPolicyOptionsWrapper = document.getElementById(
    "manual_policy_options_wrapper",
  );
  ...
  const editManualAccessGroupSelect = document.getElementById(
    "edit_manual_access_group",
  );
  const editManualPolicyOptionsWrapper = document.getElementById(
    "edit_manual_policy_options_wrapper",
  );
  if (editManualAccessGroupSelect && editManualPolicyOptionsWrapper) {
    editManualAccessGroupSelect.addEventListener("change", function () {
      ...
      editManualPolicyOptionsWrapper
        .querySelectorAll("select, input")
        .forEach((el) => {
          el.disabled = isDisabled;
        });
  ...
  document.querySelectorAll(".policy-type-select").forEach((select) => {
    ...
    const clearAccessGroupOnPolicyChange = () => {
      if (select.value && select.value !== "none") {
        const accessGroupSelect = container.querySelector(
          "#manual_access_group, #edit_manual_access_group",
        );
        if (accessGroupSelect) {
          if (accessGroupSelect.tomselect) {
            accessGroupSelect.tomselect.clear();
    ...
    button.addEventListener("click", async function () {
      ...
      if (isExpanded) {
        ...
        if (
          ...
          targetDiv.dataset.loaded === "error"
        ) {
          ...
          try {
            ...
            if (!response.ok) {
              ...
              try {
                ...
                errorDetail =
                  errorData.error || errorData.message || errorDetail;
              } catch (e) {}
              throw new Error(errorDetail);
            }
            const data = await response.json();
            const currentTargetDiv = document.getElementById(
              `dns-records-${tunnelId}`,
            );
            ...
            if (data.dns_records && data.dns_records.length > 0) {
              ...
              data.dns_records.forEach((record) => {
                ...
                const zoneDisplay = record.zone_name
                  ? record.zone_name
                  : record.zone_id;
            ...
            const errorTargetDiv = document.getElementById(
              `dns-records-${tunnelId}`,
            );
            if (errorTargetDiv) {
  ...
  const createAccessGroupBtn = document.getElementById(
    "create-access-group-btn",
  );
  ...
  if (document.getElementById("idp-table-container")) {
    ...
    document
      .getElementById("sync-idps-btn")
      ?.addEventListener("click", syncIdentityProviders);
    document.getElementById("create-idp-btn")?.addEventListener("click", () => {
      showIdPModal("create");
    });

    document
      .getElementById("idp-form")
      ?.addEventListener("submit", handleIdPFormSubmit);
    document
      .getElementById("idp-type")
      ?.addEventListener("change", updateIdPFormFields);
  }
...
function renderIdPTable(idps) {
  ...
  const typeIcons = {
    ...
    "google-apps":
      '<svg class="w-6 h-6" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>',
    azureAD:
      '<svg class="w-6 h-6" viewBox="0 0 24 24"><path fill="#00A4EF" d="M0 0h11.377v11.372H0z"/><path fill="#FFB900" d="M12.623 0H24v11.372H12.623z"/><path fill="#7FBA00" d="M0 12.628h11.377V24H0z"/><path fill="#F25022" d="M12.623 12.628H24V24H12.623z"/></svg>',
  ...
  for (const [friendlyName, idpData] of Object.entries(idps)) {
    const icon =
      typeIcons[idpData.type] ||
      '<svg class="w-6 h-6" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/></svg>';
...
async function syncIdentityProviders() {
  ...
  try {
    ...
    if (data.success) {
      ...
      await dfAlert(
        t("js.alert.sync_error", {
          error: data.error || t("js.alert.sync_error_generic"),
        }),
        t("js.alert.sync_error_title"),
      );
    }
  } catch (error) {
...
function showIdPModal(mode, friendlyName = null) {
  ...
  form.reset();
  document.getElementById("idp-config-fields").innerHTML =
...
async function handleIdPFormSubmit(e) {
  ...
  const friendlyName = document
    .getElementById("idp-friendly-name")
    .value.trim();
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.test_url && mode === "create") {
        const shouldTest = await dfConfirm(
          t("js.confirm.idp_test_success"),
          t("js.confirm.idp_test_title"),
        );
        if (shouldTest) {
      ...
      await dfAlert(
        t("js.alert.save_error", {
          error: data.error || t("js.alert.save_error_generic"),
        }),
        t("js.alert.save_error_title"),
      );
    }
  } catch (error) {
...
async function deleteIdP(friendlyName) {
  const confirmed = await dfConfirm(
    t("js.confirm.idp_delete", { friendlyName: friendlyName }),
    t("js.confirm.idp_delete_title"),
  );
  if (!confirmed) {
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      await dfAlert(
        t("js.alert.delete_error", {
          error: data.error || t("js.alert.delete_error_generic"),
        }),
        t("js.alert.delete_error_title"),
      );
    }
  } catch (error) {
    console.error("Error deleting IdP:", error);
    await dfAlert(
      t("js.alert.delete_error_generic"),
      t("js.alert.error_title"),
    );
  }
}
