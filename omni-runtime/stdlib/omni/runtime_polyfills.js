...
if (typeof globalThis.__OMNI_POLYFILLS_LOADED__ === "undefined") {
  ...
  const OmniStd = {
    Result: {
      Ok: (value) => ({
        ...
        map(f) {
          return OmniStd.Result.Ok(f(value));
        },
      }),
      Err: (error) => ({
        _tag: "Err",
        ...
        isErr: true,
        unwrap() {
          throw new Error("Unwrap on Err: " + error);
        },
        unwrapOr(fallback) {
          return fallback;
        },
        map(_) {
          return OmniStd.Result.Err(error);
        },
      }),
    },
    Option: {
      Some: (value) => ({
        ...
        map(f) {
          return OmniStd.Option.Some(f(value));
        },
      }),
      None: {
        _tag: "None",
    ...
    Vec: class {
      ...
      pop() {
        return this.items.length > 0
          ? OmniStd.Option.Some(this.items.pop())
          : OmniStd.Option.None;
      }
      len() {
      ...
      map(f) {
        return new OmniStd.Vec(this.items.map(f));
      }
      filter(f) {
        return new OmniStd.Vec(this.items.filter(f));
      }
      reduce(f, init) {
        return this.items.reduce(f, init);
      }
      forEach(f) {
    ...
    HashMap: class {
      ...
      get(key) {
        return this.entries.has(key)
          ? OmniStd.Option.Some(this.entries.get(key))
          : OmniStd.Option.None;
      }
      has(key) {
        return this.entries.has(key);
      ...
      keys() {
        return new OmniStd.Vec(Array.from(this.entries.keys()));
      }
      values() {
        return new OmniStd.Vec(Array.from(this.entries.values()));
      }
      len() {
    ...
    Queue: class {
      ...
      dequeue() {
        return this.items.length > 0
          ? OmniStd.Option.Some(this.items.shift())
          : OmniStd.Option.None;
      }
      peek() {
        return this.items.length > 0
          ? OmniStd.Option.Some(this.items[0])
          : OmniStd.Option.None;
      }
      len() {
        return this.items.length;
    ...
    Stack: class {
      ...
      peek() {
        return this.items.length > 0
          ? OmniStd.Option.Some(this.items[this.items.length - 1])
          : OmniStd.Option.None;
      }
      len() {
        return this.items.length;
    ...
    assert: (condition, msg = "Assertion failed") =>
      condition ? OmniStd.Result.Ok(undefined) : OmniStd.Result.Err(msg),
  ...
  const OmniRuntime = {
    EventLoop: {
      ...
      setTimeout: (cb, ms) => {
        const r = OmniNative.syscall("runtime_set_timeout", { delay: ms });
        if (cb) cb();
        return r.timerId;
      },
      setInterval: (cb, ms) =>
        OmniNative.syscall("runtime_set_interval", { delay: ms }).timerId,
    ...
    spawn: (task) => {
      ...
      if (task) task();
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(r.taskId);
    },
  ...
  const OmniFs = {
    readFile: (path) => {
      const r = OmniNative.syscall("fs_read", { path });
      return r.error ? OmniStd.Result.Err(r.error) : OmniStd.Result.Ok(r.data);
    },
    writeFile: (path, data) => {
      const r = OmniNative.syscall("fs_write", { path, data });
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(undefined);
    },
    exists: (path) => OmniNative.syscall("fs_exists", { path }).exists,
    mkdir: (path, recursive = true) => {
      const r = OmniNative.syscall("fs_mkdir", { path, recursive });
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(undefined);
    },
    readdir: (path) => {
      const r = OmniNative.syscall("fs_readdir", { path });
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(new OmniStd.Vec(r.entries));
    },
    remove: (path) => {
      const r = OmniNative.syscall("fs_remove", { path });
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(undefined);
    },
    stat: (path) => {
      const r = OmniNative.syscall("fs_stat", { path });
      return r.error ? OmniStd.Result.Err(r.error) : OmniStd.Result.Ok(r.stat);
    },
    glob: (pattern, dir = ".") => {
      const r = OmniNative.syscall("fs_glob", { pattern, dir });
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(new OmniStd.Vec(r.files));
    },
  };
  ...
  const OmniEnv = {
    get: (key) => {
      const r = OmniNative.syscall("std_env_get", { key });
      return r.value ? OmniStd.Option.Some(r.value) : OmniStd.Option.None;
    },
    ...
    isProduction: () =>
      OmniEnv.getOrDefault("OMNI_ENV", "development") === "production",
  ...
  const OmniCrypto = {
    sha256: (data) =>
      OmniNative.syscall("std_crypto_hash", { algo: "sha256", payload: data })
        .result,
    md5: (data) =>
      OmniNative.syscall("std_crypto_hash", { algo: "md5", payload: data })
        .result,
    uuid: () => OmniNative.syscall("std_crypto_uuid", {}).uuid,
    randomBytes: (size = 32) =>
      OmniNative.syscall("crypto_random", { size }).bytes,
    hmac: (key, data, algo = "sha256") =>
      OmniNative.syscall("crypto_hmac", { key, data, algo }).result,
    encrypt: (plaintext, key) => {
      const r = OmniNative.syscall("crypto_encrypt", {
        plaintext,
        key,
        algo: "aes-256-gcm",
      });
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(r.ciphertext);
    },
    decrypt: (ciphertext, key) => {
      const r = OmniNative.syscall("crypto_decrypt", {
        ciphertext,
        key,
        algo: "aes-256-gcm",
      });
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(r.plaintext);
    },
    hash: (data, algo = "sha256") =>
      OmniNative.syscall("std_crypto_hash", { algo, payload: data }).result,
  ...
  const OmniNet = {
    HttpResponse: class {
      ...
      json() {
        try {
          return OmniStd.Result.Ok(JSON.parse(this.body));
        } catch (e) {
          return OmniStd.Result.Err("Invalid JSON");
        }
      }
      ok() {
    ...
    fetch: (url, options = {}) => {
      const r = OmniNative.syscall("http_fetch", {
        url,
      ...
      return r.error
        ? OmniStd.Result.Err(r.error)
        : OmniStd.Result.Ok(
            new OmniNet.HttpResponse(r.status, r.headers, r.body),
          );
    },
    ...
    WebSocket: class {
      ...
      static connect(url) {
        const r = OmniNative.syscall("ws_connect", { url });
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(new OmniNet.WebSocket(url));
      }
      send(data) {
        OmniNative.syscall("ws_send", { data });
      }
      close() {
        OmniNative.syscall("ws_close", {});
      }
    },
  };
  ...
  const OmniDb = {
    Database: class {
      ...
      static connect(driver, connectionString) {
        const r = OmniNative.syscall("db_connect", {
          driver,
          connectionString,
        });
      ...
      query(sql, params = []) {
        const r = OmniNative.syscall("db_query", { sql, params });
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(new OmniStd.Vec(r.rows));
      }
      execute(sql, params = []) {
        const r = OmniNative.syscall("db_execute", { sql, params });
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(r.affected);
      }
      close() {
        OmniNative.syscall("db_close", {});
      }
    },
    Model: class {
      ...
      findById(id) {
        return this.db
          ...
          .map((rows) =>
            rows.len() > 0
              ? OmniStd.Option.Some(rows.items[0])
              : OmniStd.Option.None,
          );
      }
      create(data) {
        const r = OmniNative.syscall("db_insert", { table: this.table, data });
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(r.record);
      }
      update(id, data) {
        const r = OmniNative.syscall("db_update", {
          table: this.table,
          id,
          data,
        });
      ...
      delete(id) {
        return this.db
          .execute("DELETE FROM " + this.table + " WHERE id = ?", [id])
          .map((_) => undefined);
      }
    },
  ...
  const OmniCache = {
    Cache: class {
      ...
      static new(maxSize, defaultTTL) {
        return new OmniCache.Cache(maxSize, defaultTTL);
      }
      get(key) {
  ...
  const OmniStream = {
    Stream: class {
      ...
      static create(topic) {
        return new OmniStream.Stream(topic);
      }
      produce(data) {
        OmniNative.syscall("stream_produce", { topic: this.topic, data });
      ...
      pipe(target) {
        this.consume((data) => target.produce(data));
      }
    },
    PubSub: class {
      constructor() {
  ...
  const OmniAuth = {
    JWT: {
      sign: (payload, secret, expiresIn = 3600) => {
        ...
        const segments = [
          OmniCrypto.base64(JSON.stringify(header)),
          OmniCrypto.base64(JSON.stringify(claims)),
        ];
      ...
      verify: (token, secret) => {
        ...
        if (expectedSig !== parts[2])
          return OmniStd.Result.Err("Invalid signature");
        try {
          const payload = JSON.parse(OmniCrypto.base64Decode(parts[1]));
          if (payload.exp && payload.exp < Date.now() / 1000)
            return OmniStd.Result.Err("Token expired");
          return OmniStd.Result.Ok(payload);
        } catch (e) {
          return OmniStd.Result.Err("Invalid payload");
        }
      },
      decode: (token) => {
        const parts = token.split(".");
        if (parts.length !== 3) return OmniStd.Result.Err("Invalid JWT format");
        try {
          return OmniStd.Result.Ok(
            JSON.parse(OmniCrypto.base64Decode(parts[1])),
          );
        } catch (e) {
          return OmniStd.Result.Err("Invalid payload");
        }
    ...
    verifyPassword: (password, hash) =>
      OmniAuth.hashPassword(password) === hash,
  };
  ...
  const OmniGraphQL = {
    GraphQLSchema: class {
      ...
      static new() {
        return new OmniGraphQL.GraphQLSchema();
      }
      type(name, fields) {
        this.types.push({ name, fields });
      }
      query(name, resolver) {
        this.queries.set(name, resolver);
      }
      mutation(name, resolver) {
        this.mutations.set(name, resolver);
      }
      execute(query, variables = {}) {
        return OmniNative.syscall("graphql_execute", {
          schema: this,
          query,
          variables,
        });
  ...
  const OmniQueue = {
    JobQueue: class {
      ...
      static new(name, maxRetries) {
        return new OmniQueue.JobQueue(name, maxRetries);
      }
      process(jobType, handler) {
        this.handlers.set(jobType, handler);
      }
      dispatch(jobType, payload) {
        const r = OmniNative.syscall("queue_dispatch", {
          name: jobType,
          payload,
        ...
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(r.jobId);
      }
      schedule(jobType, payload, delayMs) {
        return OmniNative.syscall("queue_schedule", {
          name: jobType,
          payload,
          delay: delayMs,
        });
      ...
      stats() {
        return OmniNative.syscall("queue_stats", { name: this.name });
      }
    },
  };
  ...
  const OmniLog = {
    ...
    Logger: class {
      ...
      static new(name, level) {
        return new OmniLog.Logger(name, level);
      }
      debug(msg, data = {}) {
        this._log(LogLevel.DEBUG, "🔍", msg, data);
      }
      info(msg, data = {}) {
        this._log(LogLevel.INFO, "ℹ️", msg, data);
      }
      warn(msg, data = {}) {
        this._log(LogLevel.WARN, "⚠️", msg, data);
      }
      error(msg, data = {}) {
      ...
      _log(level, icon, msg) {
        if (level >= this.level)
          OmniStd.println(icon + " [" + this.name + "] " + msg);
      }
      child(name) {
        return new OmniLog.Logger(this.name + ":" + name, this.level);
      }
    },
  };
  ...
  const OmniConfig = {
    Config: class {
      ...
      static load(path) {
        ...
        try {
          ...
          for (const key of Object.keys(parsed))
            config.data.set(key, parsed[key]);
          return OmniStd.Result.Ok(config);
        } catch (e) {
          return OmniStd.Result.Err("Invalid JSON config");
        }
      }
      get(key) {
        return this.data.has(key)
          ? OmniStd.Option.Some(this.data.get(key))
          : OmniStd.Option.None;
      }
      getOrDefault(key, fallback) {
      ...
      merge(other) {
        for (const [k, v] of other.data) this.data.set(k, v);
        return this;
      }
    },
  };
  ...
  const OmniValidate = {
    Schema: class {
      constructor(rules = [], fieldName = "") {
        this.rules = [...rules];
        this.fieldName = fieldName;
      }
      static string(name = "") {
        return new OmniValidate.Schema([{ type: "string" }], name);
      }
      static number(name = "") {
        return new OmniValidate.Schema([{ type: "number" }], name);
      }
      static boolean(name = "") {
        return new OmniValidate.Schema([{ type: "boolean" }], name);
      }
      required() {
      ...
      min(val) {
        this.rules.push({ rule: "min", value: val });
        return this;
      }
      max(val) {
        this.rules.push({ rule: "max", value: val });
        return this;
      }
      email() {
        this.rules.push({ rule: "email" });
        return this;
      }
      validate(data) {
        const errors = [];
        for (const rule of this.rules) {
          if (rule.rule === "required" && (data === null || data === undefined))
            errors.push(this.fieldName + " is required");
          if (rule.rule === "min" && data < rule.value)
            errors.push(this.fieldName + " must be >= " + rule.value);
          if (rule.rule === "max" && data > rule.value)
            errors.push(this.fieldName + " must be <= " + rule.value);
          if (rule.rule === "email" && !String(data).includes("@"))
            errors.push(this.fieldName + " must be a valid email");
        }
        return errors.length > 0
          ? OmniStd.Result.Err(errors)
          : OmniStd.Result.Ok(data);
      }
    },
    sanitize: (input) =>
      String(input)
  ...
  const OmniAI = {
    AI: {
      infer: (model, input, options = {}) => {
        const r = OmniNative.syscall("ai_infer", { model, input, options });
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(r.output || r);
      },
      embed: (text, model = "default") => {
        const r = OmniNative.syscall("ai_embed", { text, model });
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(new OmniStd.Vec(r.embedding || []));
      },
      generate: (prompt, options = {}) => {
        const r = OmniNative.syscall("ai_generate", { prompt, ...options });
        return r.error
          ? OmniStd.Result.Err(r.error)
          : OmniStd.Result.Ok(r.text || "");
      },
    },
    Pipeline: class {
      constructor() {
      ...
      run(input) {
        let result = input;
        for (const step of this.steps) result = step.processor(result);
        return OmniStd.Result.Ok(result);
      }
    },
  ...
  const OmniMonitor = {
    Metrics: class {
      ...
      static new() {
        return new OmniMonitor.Metrics();
      }
    ...
    HealthCheck: class {
      ...
      static new() {
        return new OmniMonitor.HealthCheck();
      }
      add(name, checker) {
      ...
      run() {
        ...
        for (const check of this.checks) {
          ...
          if (r.isOk)
            results.push({ name: check.name, status: "OK", message: r.value });
          else {
            results.push({
              name: check.name,
              status: "FAIL",
  ...
  const OmniCli = {
    CLI: class {
      ...
      static new(name, version) {
        return new OmniCli.CLI(name, version);
      }
      command(name, desc, handler) {
        this.commands.set(name, { name, description: desc, handler });
      }
      run(argv) {
        if (argv.length < 1) {
          this.printHelp();
        ...
        if (cmd) cmd.handler({});
          ...
          OmniStd.println("Unknown command: " + argv[0]);
          this.printHelp();
        }
      }
      printHelp() {
        ...
        for (const [, cmd] of this.commands)
          OmniStd.println("  " + cmd.name + "  " + cmd.description);
      }
    },
  ...
  const OmniTypes = {
    Money: class {
      ...
      static new(amount, currency) {
        return new OmniTypes.Money(amount, currency);
      }
      add(other) {
        return this.currency !== other.currency
          ? OmniStd.Result.Err("Currency mismatch")
          : OmniStd.Result.Ok(
              new OmniTypes.Money(this.amount + other.amount, this.currency),
            );
      }
      subtract(other) {
        return this.currency !== other.currency
      ...
      multiply(factor) {
        return new OmniTypes.Money(this.amount * factor, this.currency);
      }
      format() {
    ...
    Duration: class {
      ...
      static fromSeconds(s) {
        return new OmniTypes.Duration(s * 1000);
      }
      static fromMinutes(m) {
        return new OmniTypes.Duration(m * 60000);
      }
      static fromHours(h) {
        return new OmniTypes.Duration(h * 3600000);
      }
      toSeconds() {
        return this.ms / 1000;
      }
  ...
  const OmniTest = {
    ...
    it: (name, test) => {
      try {
        ...
        OmniStd.println("    ❌ " + name + " — " + e.message);
      }
    },
    expect: (value) => ({
      toBe: (expected) => {
      ...
      toBeTrue: () => {
        if (value !== true) throw new Error("Expected true");
      },
      toBeFalse: () => {
        if (value !== false) throw new Error("Expected false");
      },
      toBeNull: () => {
      ...
      toBeGreaterThan: (n) => {
        if (!(value > n)) throw new Error("Expected > " + n);
      },
      toBeLessThan: (n) => {
        if (!(value < n)) throw new Error("Expected < " + n);
      },
      toHaveLength: (n) => {
        if (value.length !== n)
          throw new Error("Expected length " + n + ", got " + value.length);
      },
    }),
