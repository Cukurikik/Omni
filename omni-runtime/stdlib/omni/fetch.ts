 ...
 * ```
 */
export function omniFetch<T = any>(
  url: string,
  options?: OmniFetchOptions,
): OmniFetchParsed<T> {
  ...
  if (!result) {
    throw new Error(
      "[OMNI-NEXUS] Syscall returned null — Golang mungkin crash",
    );
  }
...
/** POST JSON — shorthand */
export function omniPost<T = any>(
  url: string,
  body: unknown,
  headers?: Record<string, string>,
): OmniFetchParsed<T> {
...
export function omniDelete<T = any>(
  url: string,
  headers?: Record<string, string>,
): OmniFetchParsed<T> {
...
export function omniPatch<T = any>(
  url: string,
  body: unknown,
  headers?: Record<string, string>,
): OmniFetchParsed<T> {
 ...
 */
export function omniFetchUnsafe<T = any>(
  url: string,
  options?: OmniFetchOptions,
): OmniFetchParsed<T> {
...
export function isOmniFetchOk<T>(
  response: OmniFetchParsed<T>,
): response is OmniFetchParsed<T> & { ok: true } {
  return response.ok;
}
