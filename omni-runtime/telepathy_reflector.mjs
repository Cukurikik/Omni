...
if (!fs.existsSync(pkgPath)) {
    console.error("FATAL: package.json tidak ditemukan saat Inisialisasi Reflector!");
    process.exit(1);
}
...
const bootstrap = async () => {
    ...
    console.log(`[UAST] 🚀 Berhasil menelan ${dependencies.length - failed} Paket NPM ke dalam Memori OMNI!`);
                ...
                try {
                    ...
                    */
                    ...
                    if (payload.action === "CALL_INSTANCE") {
                        ...
                        if (!instance) {
                            res.writeHead(404);
                            return res.end(JSON.stringify({ error: `Instance ${payload.instance_id} tidak ditemukan.` }));
                        }
                        const result = await instance[payload.functionality](...(payload.args || []));
                    ...
                    if (!targetPkg) {
                        res.writeHead(404);
                        return res.end(JSON.stringify({ error: `Package ${payload.package} tidak ditemukan dalam Memori UAST OMNI` }));
                    }
                    ...
                    if (typeof targetUnit !== 'function') {
                        res.writeHead(500);
                        return res.end(JSON.stringify({ error: `${payload.functionality} bukanlah sebuah fungsi/Class.`}));
                    }
                    ...
                    if (payload.action === "NEW") {
                        ...
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        return res.end(JSON.stringify({ status: "Ok", instance_id: iid, message: "Class diinstansiasi di Memory Reflector" }));
                    }
                    ...
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ status: "Ok", data: result, latency: "0.005ms" }));
                } catch (e) {
                    res.writeHead(500);
                    res.end(JSON.stringify({ error: e.message || "Internal Node Error" }));
                }
            });
        }
    });
