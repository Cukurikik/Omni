...
impl PtrPrintMap {
    ...
    fn map_id(&self, id: impl Into<u64>) -> *const c_void {
        self.map_ptr(id.into() as *const c_void)
    }

    /// Map an index into a Ruby object (e.g. for an ivar) for printing
    fn map_index(&self, id: impl Into<u64>) -> *const c_void {
        self.map_ptr(id.into() as *const c_void)
    }
            ...
            match opcode {
                ...
                YARVINSN_opt_duparray_send => {
                    ...
                    let bop = match method_id {
                        x if x == ID!(include_p).0 as u64 => BOP_INCLUDE_P,
                        _ => {
