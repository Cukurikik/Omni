...
pub fn rust_str_to_ruby(str: &str) -> VALUE {
    unsafe { rb_utf8_str_new(str.as_ptr() as *const _, str.len() as std::os::raw::c_long) }
}
...
pub(crate) mod ids {
    #[cfg(windows)]
    use std::sync::atomic::AtomicU32 as AtomicID;
    #[cfg(not(windows))]
    use std::sync::atomic::AtomicU64 as AtomicID;
    ...
    macro_rules! def_ids {
        ($(name: $name:ident $(content: $content:literal)?)*) => {
            $(
                pub static $name: AtomicID = AtomicID::new(0);
            )*
            ...
            pub(crate) fn init() {
                $(
                    ...
                    $name.store(
                        unsafe { $crate::cruby::rb_intern2(ptr.cast(), content.len() as _) }.0 as _,
