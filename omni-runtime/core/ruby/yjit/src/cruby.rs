    ...
    macro_rules! def_ids {
        ($(name: $ident:ident content: $str:literal)*) => {
            $(
                ...
                pub static $ident: AtomicU32 = AtomicU32::new(0);
            )*
            ...
            pub(crate) fn init() {
                $(
                    ...
                    $ident.store(
                        unsafe { $crate::cruby::rb_intern2(ptr.cast(), content.len() as _) } as u32,
...
macro_rules! ID {
    ($id_name:ident) => {
        $crate::cruby::ids::$id_name.load(std::sync::atomic::Ordering::Relaxed) as ID
    }
}
pub(crate) use ID;
