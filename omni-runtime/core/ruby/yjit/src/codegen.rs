...
fn gen_get_ivar(
    ...
    if !comptime_receiver.heap_object_p() || comptime_receiver.shape_too_complex() || megamorphic {
        ...
        let ivar_val = asm.ccall(rb_ivar_get as *const u8, vec![recv, Opnd::UImm(ivar_name as u64)]);
        ...
        Some(ivar_index) => {
            let ivar_opnd = if receiver_t_object {
                ...
                if assume_single_ractor_mode(jit, asm) {
                    asm.ccall(rb_ivar_get_at_no_ractor_check as *const u8, vec![recv, Opnd::UImm((ivar_index as u64).into())])
                } else {
                    // The function could raise RactorIsolationError.
                    jit_prepare_non_leaf_call(jit, asm);
                    asm.ccall(rb_ivar_get_at as *const u8, vec![recv, Opnd::UImm((ivar_index as u64).into()), Opnd::UImm(ivar_name as u64)])
                }
            };
...
fn gen_getinstancevariable(
    ...
    let ivar_name = jit.get_arg(0).as_u64() as ID;
...
fn gen_set_ivar(
    ...
    let ivar_index = if !comptime_receiver.special_const_p() && !shape_too_complex {
        let shape_id = comptime_receiver.shape_id_of();
        let mut ivar_index: u16 = 0;
        if unsafe { rb_shape_get_iv_index(shape_id, ivar_name as ID, &mut ivar_index) } {
            Some(ivar_index as usize)
        } else {
            None
        }
...
fn gen_definedivar(
    ...
    let ivar_exists = unsafe {
        let mut ivar_index: u16 = 0;
        rb_shape_get_iv_index(shape_id, ivar_name as ID, &mut ivar_index)
    };
...
fn gen_opt_duparray_send(
    ...
    let method = jit.get_arg(1).as_usize() as ID;
...
unsafe extern "C" fn build_kwhash(ci: *const rb_callinfo, sp: *const VALUE) -> VALUE {
    ...
    let hash = rb_hash_new_with_size(kw_len as std::os::raw::c_ulong);
    ...
    let kwrest_type = if has_kwrest {
        c_callable! {
            fn build_kw_rest(rest_mask: u64, stack_kwargs: *const VALUE, keywords: *const rb_callinfo_kwarg) -> VALUE {
                ...
                let hash = unsafe { rb_hash_new_with_size(keyword_len as std::os::raw::c_ulong) };
...
fn gen_struct_aref(
    ...
    assert!((off as std::os::raw::c_long) < unsafe { RSTRUCT_LEN(comptime_recv) });
...
fn get_method_name(class: Option<VALUE>, mid: ID) -> String {
