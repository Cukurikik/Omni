...
func (p *ProtocolHandler) Decode(r io.Reader) (*Message, error) {
	...
	if _, err := io.ReadFull(r, header); err != nil {
		return nil, fmt.Errorf("omni-lodash-native: protocol decode header: %w", err)
	}
	length := binary.BigEndian.Uint32(header[1:5])
	if length > p.MaxMsgSize {
		return nil, fmt.Errorf("omni-lodash-native: message too large: %d", length)
	}
	payload := make([]byte, length)
	if _, err := io.ReadFull(r, payload); err != nil {
		return nil, fmt.Errorf("omni-lodash-native: protocol decode payload: %w", err)
	}
