...
func WithRetry(ctx context.Context, cfg RetryConfig, fn func() error) error {
	...
	for i := 0; i < cfg.MaxAttempts; i++ {
		...
		case <-ctx.Done(): return fmt.Errorf("omni-coinbase-commerce: retry cancelled: %w", ctx.Err())
		}
	}
	return fmt.Errorf("omni-coinbase-commerce: max retries exceeded: %w", lastErr)
}