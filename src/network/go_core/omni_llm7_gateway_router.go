...
type ProviderConfig struct { Name string; Endpoint string; Weight int }
type Gateway struct { providers []ProviderConfig; counter uint64 }
...
func (g *Gateway) Route(model string) (*ProviderConfig, error) {
	if len(g.providers) == 0 { return nil, errors.New("OMNI_ERR: no providers") }
