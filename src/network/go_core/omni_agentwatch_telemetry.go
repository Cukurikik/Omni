...
type TraceEvent struct { AgentID string; Action string; LatencyMs float64; Tokens int; Ts int64 }
type Collector struct { mu sync.Mutex; events []TraceEvent }
...
func (c *Collector) Stats() (int, float64, error) {
	c.mu.Lock(); defer c.mu.Unlock()
	if len(c.events) == 0 { return 0, 0, errors.New("OMNI_ERR: no events") }
	total := 0.0
	for _, e := range c.events { total += e.LatencyMs }
