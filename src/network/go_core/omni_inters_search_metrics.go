...
func MRR(ranked []string, relevant map[string]bool) float64 {
	for i, r := range ranked {
		if relevant[r] { return 1.0 / float64(i+1) }
	}
	return 0
}
...
func MAP(rankedLists [][]string, relevantSets []map[string]bool) float64 {
	...
	for q, ranked := range rankedLists {
		ap := 0.0; hits := 0
		for i, r := range ranked {
			if relevantSets[q][r] { hits++; ap += float64(hits) / float64(i+1) }
		}
		if len(relevantSets[q]) > 0 { total += ap / float64(len(relevantSets[q])) }
	}
	if len(rankedLists) == 0 { return 0 }
