...
func (t *Trace) AddSpan(name string, latency float64, tokens int) {
	t.Spans = append(t.Spans, Span{name, latency, tokens})
}
func (t Trace) TotalCost(costPerToken float64) float64 {
	total := 0.0
	for _, s := range t.Spans {
		total += float64(s.Tokens) * costPerToken
	}
	return total
}
func (t Trace) AvgLatency() float64 {
	...
	return sum / float64(len(t.Spans))
}
