    ...
    def record(self, duration_sec: float = 3.0) -> list:
        ...
        for i in range(num_samples):
            ...
            sample = (
                ...
                0.05 * math.sin(2 * math.pi * 37 * t)   # Deterministik
            )
    ...
    def decode(self, mel_spec: list) -> dict:
        ...
        for i, word in enumerate(text.split()):
            words_with_timestamps.append({
                ...
                "confidence": round(confidence - (i % 5) * 0.01, 3)
            })
...
class OmniLLMRouter:
    """
    LLM Router — PRODUCTION: panggil Ollama API nyata.
    Fallback ke keyword-based intent classification jika Ollama offline.
    """
    ...
    def __init__(self, ollama_url="http://localhost:11434", model="llama3.2"):
        ...
        status = "LIVE" if self._ollama_live else "OFFLINE (keyword fallback)"
        print(f"🧠 [LLM] Router diinisiasi ({self.ollama_url}) — {status}")
    ...
    def generate_response(self, text: str) -> str:
        """Generate respons — PRODUCTION: panggil Ollama API, fallback keyword."""
        if self._ollama_live:
            try:
                payload = json.dumps({
                    "model": self.model,
                    "prompt": f"Kamu adalah Omni AI voice assistant. User berkata: '{text}'. Berikan respons singkat.",
                    "stream": False,
                }).encode()
                ...
                with urllib.request.urlopen(req, timeout=15) as resp:
                    ...
                    print(f"   ✅ [PRODUCTION] Ollama: {content[:60]}...")
                    return content
            except Exception as e:
                print(f"   ⚠️ Ollama error: {e}. Falling back.")
