...
type AdapterResult struct { IsOk bool; Error error }
...
func (p *AdapterWorkerPool) InjectLoRA(layerIdx int, rank int) AdapterResult {
	if rank > 256 { return AdapterResult{false, errors.New("LoRA rank exceeds 256")} }
	cur := atomic.AddInt32(&p.active, 1)
	if cur > MAX_ADAPTERS { atomic.AddInt32(&p.active, -1); return AdapterResult{false, errors.New("adapter pool full")} }
