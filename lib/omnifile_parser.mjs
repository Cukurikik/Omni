...
export function toAppConfig(omniConfig) {
  return {
    ...
    nexus_cluster: {
      ...
      cluster_secret: omniConfig.NEXUS_SECRET || "OMNI_NEXUS_DEFAULT_KEY",
      worker_max_jobs: 2,
...
export function generateOmnifileContent(config) {
  ...
  content += "# ===================================\n\n";
  content += "# PROTOKOL PIHAK KETIGA\n";
  content += 'THIRD_PARTY_INTEGRATION "FALSE"\n\n';
  content += "# EXTERNAL_MODULES:\n";
  content += "#   - stripe_payment\n\n";
  ...
  // Array blocks
  const arrayKeys = Object.keys(config).filter((k) => Array.isArray(config[k]));
