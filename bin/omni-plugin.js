...
module.exports = {
  ...
  run: async (payload, api) => {
    ...
    console.log(
      `✅ Plugin ${pluginId} berhasil dibuat. Silakan modifikasi plugin.json dan index.js`,
    );
  });
...
program
  ...
  .action((pluginDir) => {
    ...
    const outPath = path.join(
      process.cwd(),
      `${path.basename(pluginDir)}-release.zip`,
    );
    zip.writeZip(outPath);
    console.log(
      `✅ Plugin dibundle ke ${outPath}. Siap diupload! (Integrity Signature: Skipped in Dev Mode)`,
    );
  });
